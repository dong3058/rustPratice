...
// 즉 컴파일 시점에서 애가 T타입의 집합체인건 알겠는대 그게 어떤 형태의 집합체인지를 모른다=-->메타데이터를 작성할수 없다 이말이된다.
...
//box는 유용한 스마트 포인터인대
//Box::new(5); 를 하면은 스택에 변수정보만 저장되고 box내부의 값은 힙메모리에 저장된다 즉 5라는 원시타입도 힙에 저장되는것.
//즉 모든 변수를 힙에 저장하고 스 메타데이터만 스택에 저장하는꼴로 애는 크기를 예측할수없는 [i32]나 재귀적 구조체 구조에대해서
//에러를 안일으키게 만들어준다.

struct Node {
    next_node: Option<Box<Node>>,
}
//다음의 노드의 경우 box없이 사용하면 node의 크기를 알기위해서 node를 알아야되는 무한 재귀로인해 에러가일어난다-->크기를 예측할수없기떄문
//그러나 box를 사용하면 node라는 데이터가 힙메모리에 저장되고 option안에는 스마트 포인터가 저장된다.-->크기 예측가능.
...
//동적 바인딩의 예시인대 이것도 원래는 return이 뭐가될지 즉 human인지 animal인지 결정할수 없으나 box로 써버리면 가능하다.

/* 
let b1 = Box::new([1,2,3,4]);
let b2 = b1; 
다음과 같은 코드는 배열은 본디 stack에만 저장되기에 변수이동시 copy가 일어나나 box로 보내려미녀 추가적으로 배열을 만다는게아닌
소유권의 이동으로 좀더빨라진다.
*/